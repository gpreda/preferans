...
class PlayingStyle(Enum):
    AGGRESSIVE = "aggressive"   # Alice: optimizes for high scores
    CAUTIOUS = "cautious"       # Bob: optimizes for fewer failed games
    PRAGMATIC = "pragmatic"     # Carol: takes calculated risks
...
class Player:
    ...
    playing_style: PlayingStyle = PlayingStyle.PRAGMATIC
...
@dataclass
class Alice(Player):
    """Aggressive player — optimizes for high scores."""
    name: str = "Alice"
    playing_style: PlayingStyle = PlayingStyle.AGGRESSIVE


@dataclass
class Bob(Player):
    """Cautious player — optimizes for fewer failed games."""
    name: str = "Bob"
    playing_style: PlayingStyle = PlayingStyle.CAUTIOUS


@dataclass
class Carol(Player):
    """Pragmatic player — takes calculated risks."""
    name: str = "Carol"
    playing_style: PlayingStyle = PlayingStyle.PRAGMATIC
...
class Round:
    ...
    # Whisting phase fields
    whist_current_id: Optional[int] = None
    whist_declarations: dict = field(default_factory=dict)   # {player_id: action}
    whist_pending: list[int] = field(default_factory=list)   # players still to declare
    whist_followers: list[int] = field(default_factory=list) # players who followed/called
    whist_declaring_done: bool = False   # True after initial follow/pass declarations
    declarer_responding: bool = False    # True when declarer responds to a counter
    has_counter: bool = False            # True if a counter was declared
    has_double_counter: bool = False     # True if declarer doubled the counter
    counter_player_id: Optional[int] = None  # ID of the player who countered
    ...
    def to_dict(self, hide_talon: bool = True) -> dict:
        return {
            ...
            "phase": self.phase.value,
            "whist_current_id": self.whist_current_id,
            "whist_declarations": {str(k): v for k, v in self.whist_declarations.items()},
            "whist_declaring_done": self.whist_declaring_done,
            "declarer_responding": self.declarer_responding,
            "has_counter": self.has_counter,
            "has_double_counter": self.has_double_counter,
        }
